...
CURRENT_USER_OBJECTID=$(az ad signed-in-user show --query "{objectID:id}" -o tsv)
...
AZURE_GROUP_ID=$(az ad group show --group "devopsjourney-aks-group" --query "{objectID:id}" -o tsv)
